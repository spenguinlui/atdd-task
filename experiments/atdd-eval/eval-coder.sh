...
GOLD_PASS=""
run_one(){
  local label="$1" mode="$2" model="${3:-}"
  local wt="$REPO/.eval-${TICKET}-${label}" base_name
  base_name=".eval-${TICKET}-${label}"
  local db; db="eval$(echo "${TICKET}${label}" | tr -cd 'a-zA-Z0-9')"
...
只修改程式碼讓需求成立。不要新增或修改任何測試檔（*_spec.rb）。"
  case "$mode" in
    ...
    claude)
      local out; out=$(cd "$wt" && claude -p "$instr" --model "$model" --output-format json --permission-mode bypassPermissions 2>/dev/null)
  ...
  docker exec "$CONT" sh -c "cd /app/$base_name && RAILS_ENV=test DATABASE_DBTEST=$db bundle exec rails db:create db:schema:load >/dev/null 2>&1; DATABASE_DBTEST=$db bundle exec rspec$SPECS" > "/tmp/eval_${label}.log" 2>&1
  local line; line=$(grep -iE '[0-9]+ examples?,' "/tmp/eval_${label}.log" | tail -1)
  local ex fa pass
