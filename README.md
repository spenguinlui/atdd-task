...
ATDD task 的核心是**逐階段驗收交付產物**——需求、規格、測試、實作、審查，每一份 deliverable 都要先通過該階段的驗收閘門，才往下一步走，用層層把關換取**穩定、可預期的開發**。你只要把需求交給 AI，並在關鍵點確認。
...
| 指令 | 說明 |
|------|------|
| `/atdd {自然語言需求}` | **自然語言入口**：用一段話描述需求，AI 自動判斷類型（feature / fix / refactor / health-check / spec-update）並啟動 |
...
| `/commit` | **僅** Git Commit、不結案（需要分階段送 commit 時用）|
| `/close [task_id]` | 結案但不部署 |
| `/deploy` | 部署框架到遠端 Server |
| `/verify [task_id]` | 確認部署後 production 正常 |
| `/escape {task_id}, {問題}` | **Production 問題回報**：已部署任務在 production 出狀況時，啟動 escalation 流程 |
...
| 指令 | 說明 |
|------|------|
| `/test-create {project}, {suite 描述}` | 建立新的 E2E 測試套件 |
...
| 指令 | 說明 |
|------|------|
| `/test-pause` | 暫停當前 E2E 測試，等待人工介入 |
| `/test-resume` | 繼續暫停的 E2E 測試 |
| `/test-fail` | 標記當前測試失敗並停止 |
| `/test-skip` | 跳過當前測試步驟或場景 |
| `/test-revise` | 修正場景預期值（系統正確、測試錯誤），暫停確認後繼續 |

#### 測試中發現問題 → 建衍生任務

測試跑出問題時，常常要拉出測試之外的工作。這組指令邊測試邊建後續任務：

| 指令 | 說明 |
|------|------|
| `/test-feature` | 記錄缺少功能、建 Feature 任務，繼續測試 |
| `/test-fix` | 記錄問題、建 Fix 任務，繼續測試 |
| `/test-fix-stop` | 記錄問題並建 Fix 任務，**停止**測試 |
| `/test-refactor` | 記錄架構問題、建 Refactor 任務，繼續測試 |
| `/test-knowledge` | 記錄知識缺口，停止測試並退回 requirement 階段 |
